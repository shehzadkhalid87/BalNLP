...
def analyze_vocabulary(tokenizer):
    ...
    special_count = sum(1 for token in vocab if token.startswith('<') and token.endswith('>'))
    print(f"Special tokens: {special_count}")

    # Count merged tokens (multi-character)
    merged_count = sum(1 for token in vocab if len(token) > 1 and not token.startswith('<'))
    print(f"Merged tokens: {merged_count}")

    # Show some merged tokens
    merged_tokens = [token for token in vocab if len(token) > 1 and not token.startswith('<')]
